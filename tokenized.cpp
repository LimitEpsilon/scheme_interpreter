...
istream& operator>>(istream& is, Tokenized& t) {
	...
	while ((c = is.get()) != ENDOFEXP) {
		if (c == ' ' || c == '\n' || c == '\t') {
			if (t.data.back() != ' ') t.data.push_back(' ');
		}
		else if (c == '(' || c == ')' || c == '\'') {
			if (t.data.back() != ' ') t.data.push_back(' ');
			t.data.push_back(c);
	...
	if (t.data.back() != ' ') t.data.push_back(' '); //If the end of the input was a symbol
	return is;
}
...
Tokenized* Tokenized::read_expression() {
	string token; int lparen = 0;
	Tokenized* expression = new Tokenized();
	token = read_token();
	if (token.empty()) return nullptr;
	else if (token == "'") { //is a quote
		expression->data += token; expression->data += " ";
	}
	...
	while (1) { //if quote or a list, read the operands
		token = read_token();
		expression->data += token; expression->data += " ";
		if (token == "(") ++lparen;
