...
private:
	...
	string &token(int i)
	{
		if (i <= 0 || i > size)
		{
			return hashtable[0].lchild;
		}
		else
		{
			return hashtable[i].lchild;
		}
	}
	int &value(int i)
	{
...
public:
	...
	T pop()
	{
		if (isEmpty())
		{
			cout << "StackTable is empty, cannot pop." << endl;
			return T();
		}
		return stack[last--];
	}
...
public:
	...
	ListTable &operator=(const ListTable &memory)
	{
		for (int i = 0; i <= size; ++i)
			list[i] = memory.list[i];
		for (int i = 0; i <= ht->size; ++i)
			ht->hashtable[i] = memory.ht->hashtable[i];
		for (int i = 0; i < STACK_SIZE; ++i)
		{
			stack->stack[i] = memory.stack->stack[i];
			gcstack->stack[i] = memory.gcstack->stack[i];
		}
		return *this;
	}
...
public:
	...
	{
		ChainNode *c = new ChainNode(h, v, nullptr);
		push(c);
	}
	ChainNode pop()
	{
